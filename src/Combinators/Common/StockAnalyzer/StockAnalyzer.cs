...
using FunctionalConcurrency;
using StockAnalyzer.CS;
...
namespace StockAnalyzer
{
    public class StockAnalyzer
    {
        ...
        async Task ProcessStockHistoryParallel()
        {
            ...
            foreach (var op in stockHistoryTasks)
            {
                using (var disposable = gate.AsyncAcquire(TimeSpan.FromSeconds(1)))
                {
        ...
        public async Task ProcessStockHistoryParallel_SOL()
        {
            var sw = Stopwatch.StartNew();
